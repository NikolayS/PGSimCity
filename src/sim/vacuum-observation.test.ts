...
it('reports exhaustion rather than inventing a checkpoint when the snapshot remains held', () => {
  ...
  journey.start()
  for (let i = 0; i < 100 && journey.checkpoints.at(-1)?.kind !== 'constrained'; i++) journey.tick()
  expect(journey.checkpoints.at(-1)?.kind).toBe('constrained')
  journey.start()
  for (let i = 0; i < 100; i++) journey.tick()
  expect(journey.status).toBe('exhausted')
  expect(journey.advanced).toBeCloseTo(900, 5)
  expect(journey.checkpoints.map(c => c.kind)).toEqual(['pinned', 'constrained'])
})

it('can stop on an actually horizon-limited sessions pass before intervention', () => {
  const sim = createSim(createBus())
  sim.runScenario('vacuum-blockade'); sim.setKnob('paused', true)
  const journey = createVacuumObservation(sim)
  const seek = () => {
    expect(journey.start()).toBe(true)
    while (journey.status === 'running') journey.tick()
  ...
  seek(); seek()
  expect(journey.checkpoints.map(c => c.kind)).toEqual(['pinned', 'constrained'])
  const ti = sim.state.tables.findIndex(t => t.def.id === 'sessions')
  expect(sim.state.autovac.workers.some(w => w.active && w.table === ti && w.stalledByHorizon)).toBe(true)
  expect(sim.state.knobs.longRunningXact).toBe(true)
  const saved = structuredClone(journey.checkpoints[1])
  expect(sim.chooseScenario('terminate-transaction')).toBe(true)
  journey.observe(); seek(); seek()
  expect(journey.checkpoints.map(c => c.kind)).toEqual(['pinned', 'constrained', 'released', 'eligible', 'collected'])
  expect(journey.checkpoints[1]).toEqual(saved)
  expect(journey.checkpoints.at(-1)!.reclaimed).toBeGreaterThan(0)
})
