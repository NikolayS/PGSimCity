...
export const SCENARIOS: ScenarioDef[] = [
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [62, 'That is the storm', 'Checkpoint → full-page writes → more WAL → earlier checkpoint. The city shows that feedback in WAL, checkpoint and I/O counters; it does not measure query latency. On PostgreSQL, the same pressure can produce periodic latency spikes.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [62, 'How you would see this', 'The city shows sampled backend writes and cache misses, but it does not measure query latency. On PostgreSQL, query `pg_stat_io` and filter on `backend_type = \'client backend\'`: a large share of writes there means shared_buffers is too small or the bgwriter is not keeping up. Before PostgreSQL 17 the related signal was `buffers_backend` in pg_stat_bgwriter.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [12, 'Direct waiters form', 'Red lines in the lock manager mark modeled backends blocked directly by the one scripted ACCESS EXCLUSIVE holder. The city counts waiters and occupied backend slots; it has no query-latency series.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [48, 'What the city can show', 'Sampled backend-write counters and red backend-path particles rise because dirty-victim write time is charged to the modeled backend. The city does not measure query latency or a p99; a real PostgreSQL deployment can expose this critical-path I/O as a long latency tail.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [48, 'Saturation is not throughput', 'All modeled slots are busy, offered load exceeds achieved TPS, and new work queues. Those are the city’s signals; it has no latency series and cannot show the real response-time cost of excess connections.'],
