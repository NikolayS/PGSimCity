...
export const SCENARIOS: ScenarioDef[] = [
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    blurb: 'Compare one thousand direct clients with the same clients queued behind a bounded PgBouncer transaction pool.',
    icon: '◉',
    focus: 'client.pooler',
    duration: 96,
    knobs: {
    ...
    beats: [
      [0, 'One thousand direct clients', 'PostgreSQL starts one operating-system process per accepted connection. Here max_connections is a sixteen-slot scale model: sixteen of the thousand clients get server backends and the rest are refused. The direct backends churn and cross the city’s disclosed, uncalibrated concurrency-pressure curve.'],
      ...
      [28, 'Read the direct baseline', 'Open the Latency vital. Pool-slot queue is zero because there is no pooler; the model’s backend pressure remains in Active / unclassified. For direct connections, the rolling measurement excludes this client-side queue, then includes the explicit transaction-pool queue after the switch. Compare achieved TPS, p50 and p99. Refused clients are failure, not fast responses.'],
      [44, 'Now bound PostgreSQL concurrency', 'PgBouncer transaction pooling is now on. max_client_conn admits the one thousand client sockets, while default_pool_size holds PostgreSQL to eight persistent server connections. A pooler does not accelerate a statement; the smaller server-side concurrency is the whole mechanism.'],
      [60, 'The wait moved to a pool slot', 'Excess transactions now wait before PostgreSQL and the Latency vital attributes that estimate to Pool-slot queue. pg_stat_activity sees only the eight server backends; PgBouncer SHOW POOLS is where client_active and client_waiting expose the larger client side. This is a different queue with a different fix.'],
