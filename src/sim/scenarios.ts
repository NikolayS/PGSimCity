...
export const SCENARIOS: ScenarioDef[] = [
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [62, 'That is the storm', 'Checkpoint → full-page writes → more WAL → earlier checkpoint. The city shows that feedback in WAL, checkpoint and I/O counters; open the Latency vital to compare modeled p50 and p99 and decompose the p99 trip. Those model-time quantiles are not production milliseconds.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [12, 'Direct waiters form', 'Red lines in the lock manager mark modeled backends blocked directly by the one scripted ACCESS EXCLUSIVE holder. The city counts waiters and occupied backend slots; the rolling p99 trip attributes its blocked time to Lock wait.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [48, 'Measure the claim', 'Open the Latency vital and compare modeled p50, p99 and TPS. The p99 anatomy separates dirty-victim-write time from buffer reads and commit waits. The instrument does not assume the lesson: at this calibration, turning the bgwriter off does not reliably raise overall p99, so the backend-write counter proves the mechanism while the production tail remains a claim to verify on PostgreSQL.'],
