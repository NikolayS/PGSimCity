...
export const SCENARIOS: ScenarioDef[] = [
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [14, 'Everything is a cache hit', 'The hit ratio sits near 99%. Postgres is not reading from disk; it is reading from its own buffer pool, and the operating system is caching whatever it misses. The green roads down to storage are quiet on purpose.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [104, 'Check your own server', 'pg_stat_bgwriter (pg_stat_checkpointer on PG17+) tells you the ratio directly. If checkpoints_req is anywhere near checkpoints_timed, you are living in this scenario.'],
    ],
  },
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [62, 'That is buffers_backend', 'In pg_stat_bgwriter it is the buffers_backend counter. If it is large relative to buffers_checkpoint, either shared_buffers is too small or the bgwriter is not keeping up. Both show up as latency, never as an error.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [16, 'sessions is the victim', 'Small table, rewritten constantly, and only 15% of its updates can be HOT. Watch its bloat bar climb in the underworld while accounts — 85% HOT — barely moves.'],
      ...
      [88, 'What a worker actually does', 'Scan the heap for dead line pointers. Then one full pass per index to remove their entries — this is why a table with six indexes is six times more expensive to vacuum. Then back to the heap to free the line pointers, then truncate trailing empty pages, then analyze.'],
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [80, 'What to tune', 'bgwriter_lru_maxpages and bgwriter_delay. If buffers_backend in pg_stat_bgwriter is a large share of total writes, the bgwriter is being outrun. Raising maxpages is nearly free.'],
    ],
  },
