...
export const SCENARIOS: ScenarioDef[] = [
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    blurb: 'An idle REPEATABLE READ transaction retains a snapshot and delays dead-version cleanup.',
    icon: '◔',
    focus: 'proc.array',
    duration: 150,
    knobs: {
    ...
    beats: [
      [0, 'A snapshot left open', 'This session ran BEGIN ISOLATION LEVEL REPEATABLE READ, then a SELECT from a table, and went idle without COMMIT or ROLLBACK. The SELECT established the snapshot it still holds. Plain BEGIN at READ COMMITTED does not do this. The scenario starts with that retained snapshot already in place.'],
      ...
      [30, 'Autovacuum still runs', 'This is the cruel part. The launcher still dispatches workers. They still travel to the table, still scan heap pages, still spend I/O. Real PostgreSQL can skip all-visible pages; this model uses a scaled table scan. And they collect almost nothing, because nothing is removable yet.'],
      ...
      [66, 'Bloat with no brakes', 'With continued updates, versions protected by the old snapshot accumulate. HOT can still create same-page versions when there is room, but pruning cannot remove versions the snapshot may need. In this model, accounts starts bloating too.'],
      [86, 'Where to look', 'Inspect pg_stat_activity.backend_xmin and backend_xid, alongside state, xact_start and query_start. An old xact_start or "idle in transaction" alone does not prove a retained snapshot. Also inspect replication-slot horizons and standby feedback; establish which horizon is actually delaying cleanup.'],
      [108, 'Release it', 'Turn off the long-running transaction knob. If this was the oldest blocker, the horizon can advance. Committed dead versions no longer protected by any horizon become eligible for cleanup; a later vacuum pass can reclaim them.'],
