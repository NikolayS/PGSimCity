...
export const SCENARIOS: ScenarioDef[] = [
  ...
  /* ---------------------------------------------------------------------- */
  {
    ...
    beats: [
      ...
      [48, 'Measure the claim', 'Open the Latency vital and compare modeled p50, p99 and TPS. With the bgwriter off at this calibration, dirty-victim-wait p99 rises from 11.1 to 36.7 model ms and total p99.9 rises from 1266.7 to 1400.0 model ms, while total p99 remains 1133.3 model ms. Only 1.00% to 1.28% of transactions ride a trip with any eviction wait, so the added cost sits just outside the 99th percentile; incidence, not 33.33 model ms integration resolution, explains the unchanged p99. The wait includes XLogFlush when the victim page LSN is ahead of durable WAL, followed by the page write.'],
