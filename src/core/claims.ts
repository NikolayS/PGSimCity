...
export const CLAIM_VALUES = {
  ...
  connectionPooler: {
    ...
    coverageDisclosure: 'PGSimCity models one PgBouncer-shaped user/database pool: client admission, a bounded count of persistent PostgreSQL server connections, an uncalibrated backend-concurrency pressure curve when application clients exceed the city\'s max_connections, and transaction-pool slot wait in the end-to-end latency decomposition. pool_mode=session uses the same server cap, but client-to-server session binding, disconnect lifetimes and all session-level SQL state are absent. The queue estimate is pending modeled transactions divided by admitted offered TPS; it is not a PgBouncer timing sample.',
  },
  workMem: {
    defaultMiB: 4,
    hashMemMultiplier: 2,
