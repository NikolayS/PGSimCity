...
export const CLAIM_VALUES = {
  ...
  restoreDrill: {
    levels: {
      table: {
        ...
        supports: 'The modeled full-cluster backup fetch and archived-WAL replay reached its recovery stop — the selected target, or the fork with quantified loss for a discarded divergent-tail target under latest — and the accounts smoke check found its expected row witness.',
      ...
      cluster: {
        ...
        supports: 'The modeled full-cluster backup fetch and archived-WAL replay reached its recovery stop — the selected target, or the fork with quantified loss for a discarded divergent-tail target under latest — and every modeled table returned its expected smoke-check row witness.',
      ...
      verified: {
        ...
        supports: 'The modeled full-cluster backup fetch and archived-WAL replay reached its recovery stop — the selected target, or the fork with quantified loss for a discarded divergent-tail target under latest — the restored-object digest matched its manifest, and every modeled table returned its expected smoke-check row witness.',
    ...
    timeDisclosure: 'Restore replay time is model time at fixed teaching rates, not production RTO. It stops when WAL replay reaches the selected target or, under latest for a discarded divergent-tail target, when recovery lands at the fork and reports the loss. It excludes promotion, recovery_target_action, endpoint cutover, client reconnection, and service restoration.',
  ...
  timelineRecovery: {
    ...
    defaultDisclosure: 'PostgreSQL 18.3 defaults recovery_target_timeline to latest. With current, PostgreSQL stays on the base backup’s timeline and replays that timeline’s archived WAL: if that WAL reaches the target, recovery succeeds; otherwise it reports that the target was not reached after replaying as far as the archive goes.',
