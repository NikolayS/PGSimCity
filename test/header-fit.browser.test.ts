...
it('keeps header identity and every tool in separate visible space', async () => {
  const reports = await inspectRenderedPages([{name:'City',path:'/',readySelector:'.hud-investigate',reducedMotion:true}], async ({evaluate,send}) => {
    ...
    const states = []
    for (const width of [1450,1536,1920,1440,980,794,390,320]) {
      ...
      states.push(await evaluate(`(async()=>{
        ...
        const elements=[...document.querySelectorAll('#hud-top .hud-tools > *,#hud-top .hud-ckpt')];
        const rects=elements.map(e=>({name:e.className,r:e.getBoundingClientRect()})).filter(x=>x.r.width&&x.r.height);
        ...
        return {requestedWidth:${width},width:innerWidth,overlaps,outside,count:rects.length,parent:document.querySelector('.hud-tools').parentElement.className};
      })()`))
    }
    return states
  })
