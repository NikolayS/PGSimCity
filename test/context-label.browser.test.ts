...
it('keeps qualified selected context readable inside narrow and desktop city space', async () => {
  const reports = await inspectRenderedPages([{ name: 'City', path: '/', readySelector: '.hud-investigate', reducedMotion: true }], async ({ evaluate, send }) => {
    await evaluate(`(async()=>{for(let i=0;i<240&&!window.PGSIMCITY;i++)await new Promise(r=>setTimeout(r,50));const p=PGSIMCITY;p.sim.setKnob('paused',true);p.gfx.setQuality('low');Array.from(document.querySelectorAll('button')).find(b=>b.textContent.trim()==='Explore freely')?.click()})()`)
    const states = []
  ...
  for (const state of reports[0]) {
    ...
    expect(state.rect.right).toBeLessThanOrEqual(state.width)
