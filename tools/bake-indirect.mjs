...
try {
  ...
  if (!ready) throw new Error('PGSimCity did not finish booting')

  const evaluated = await send('Runtime.evaluate', {
    expression: `(async () => {
      const app = window.PGSIMCITY;
      const previousQuality = app.gfx.quality.level;
      const previousPaused = app.sim.state.knobs.paused;
      const previousTheme = app.themeMode();
      const offQuality = app.bus.on('quality', ({ level }) => {
        if (level !== 'high') app.gfx.setQuality('high');
      });
      let payload;
      try {
        app.sim.setKnob('paused', true);
        app.setThemeMode('day', { persist: false });
        app.gfx.setQuality('high');
        const module = await import('/src/world/baked-light.ts?offline-bake');
        if (app.gfx.quality.level !== 'high') throw new Error('bake quality is not high');
        payload = module.bakeSceneIndirect(app.gfx.scene);
        if (app.gfx.quality.level !== 'high') throw new Error('bake quality changed while sampling');
      } finally {
        offQuality();
        app.gfx.setQuality(previousQuality);
        app.sim.setKnob('paused', previousPaused);
        app.setThemeMode(previousTheme, { persist: false });
      }
      if (app.gfx.quality.level !== previousQuality) throw new Error('bake did not restore quality');
      return JSON.stringify({ payload, sampledQuality: 'high', restoredQuality: app.gfx.quality.level });
    })()`,
    awaitPromise: true,
    returnByValue: true,
  })
  ...
  const staged = JSON.parse(evaluated.result.value)
...
`
  ...
  console.log(JSON.stringify({
    ...
    sampledQuality: staged.sampledQuality,
    restoredQuality: staged.restoredQuality,
  }))
} finally {
  await run.cleanup()
